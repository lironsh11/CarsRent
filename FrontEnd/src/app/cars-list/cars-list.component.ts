...
export class CarsListComponent implements OnInit {
  ...
  constructor(private Connection: HttpClient, 
              private Connection2: HttpClient,
  ...
  PostNewRent(rentDate: Date, returnDate: string) {
    if (this.errorMsg(returnDate)) {
      ...
      this.postConnection.post("http://localhost:64130/rentinfo", {
      ...
      this.ConnectionPut.put("http://localhost:64130/Carsinfo/" + this.currentCar.carId + "/no", { "observe": "response" }).subscribe();
