...
export class ReturnCarComponent implements OnInit {
  ...
  calcRentPrice() {
    for (let i = 0; i < this.arrCarInfo.length; i++) {
      if (this.arrCarInfo[i].carId == this.currentRent.carId) {
        this.currentCarDayValue = this.arrCarInfo[i].dayValue
        this.currentCarLateValue = this.arrCarInfo[i].lateDayValue
      }
    }
    let currentDate = new Date(this.currentRent.rentDate);
    let returnDate = new Date(this.currentRent.returnDate);
    let realReturnDate = new Date(this.currentRent.realReturnDate);
    if (this.isAfterReturn) {
      this.currentRent.realReturnDate = this.todayDate;
      realReturnDate = new Date(this.todayDate);
    }
    //calculate the fine if the user return the car late
    if (this.currentRent.returnDate != this.currentRent.realReturnDate && this.currentRent.realReturnDate != null && this.currentRent.returnDate < this.currentRent.realReturnDate) {
      ...
      let fine = daysFine * parseInt(this.currentCarLateValue);
      ...
      this.finalPrice *= parseInt(this.currentCarDayValue) + fine;
    }
    else {
      this.finalPrice = Math.floor((Date.UTC(returnDate.getFullYear(), returnDate.getMonth(), returnDate.getDate()) - Date.UTC(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate())) / (1000 * 60 * 60 * 24));
      this.finalPrice *= parseInt(this.currentCarDayValue);
    }
  }
  ...
  GetRentInfo(carid: number): void {
    for (let i = 0; i < this.arrRentInfo.length; i++) {
      if (this.arrRentInfo[i].carId == carid) {
        ...
        if (this.arrRentInfo[i].realReturnDate == null ||this.arrRentInfo[i].realReturnDate == "null"|| this.arrRentInfo[i].realReturnDate == "") {
          this.isCarReturned = false
          this.currentRent = this.arrRentInfo[i];
